...
void BLECharacteristic::notify() {
  ...
  for (uint8_t i = 0; i < client_count; i++) {
    ...
    if (entry == nullptr) {
      ESP_LOGW(TAG, "Notification skipped for handle 0x%04X: client %u has not enabled its CCCD", this->handle_,
               client);
      continue;
    }
...
void BLECharacteristic::set_notify_for_client(uint16_t conn_id, bool notify, bool indicate) {
...
void BLECharacteristic::add_descriptor(BLEDescriptor *descriptor) {
  ...
  if (descriptor->get_uuid() == ESPBTUUID::from_uint16(ESP_GATT_UUID_CHAR_CLIENT_CONFIG)) {
    descriptor->on_write([this](std::span<const uint8_t> value, uint16_t conn_id) {
      ...
      ESP_LOGI(TAG, "Client %u configured notifications for handle 0x%04X: notify=%s, indicate=%s", conn_id,
               this->handle_, YESNO(notify), YESNO(indicate));
      this->set_notify_for_client(conn_id, notify, indicate);
    });
  }
...
void BLECharacteristic::gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
  ...
  switch (event) {
    case ESP_GATTS_ADD_CHAR_EVT: {
      // A HID service legitimately contains multiple Report characteristics,
      // all with UUID 0x2A4D. Only the characteristic currently being created
      // may consume this completion event. Matching on UUID alone makes later
      // duplicate characteristics dereference a service pointer before their
      // own do_create() has run.
      if (this->state_ == CREATING && this->service_ != nullptr &&
          this == this->service_->get_last_created_characteristic() &&
          this->uuid_ == ESPBTUUID::from_uuid(param->add_char.char_uuid)) {
